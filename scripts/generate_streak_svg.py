...
for i, c in enumerate(contribs):
    ...
    anim_tags = (
        ...
        f'begin="{delay}s" dur="{DUR}s" fill="freeze" calcMode="spline" keySplines="0.2 0.8 0.2 1"/>'
    )
    
    rects.append(
        f'<g opacity="0" transform="translate(0, -6)">'
        f'{anim_tags}'
        f'<rect x="{x}" y="{y}" width="{CELL}" height="{CELL}" rx="{RAD}" fill="{COLORS[lvl]}"/></g>'
    )
...
<text x="{W/2}" y="{TITLEBAR_H/2 + 4}" fill="{GRAY}" font-size="12" text-anchor="middle">virendra@github: ~/contributions --graph</text>
{''.join(labels)}
{''.join(rects)}
</svg>'''
